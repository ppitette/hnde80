...
first-install: docker-up composer-install sf-perm sf-dc sf-dmm sf-start ## First install.
.PHONY: first-install

# start: docker-up db-backup sf-start sf-open ## Start project.
start: docker-up sf-start ## Start project.
.PHONY: start

# stop: db-backup docker-stop sf-stop ## Stop project.
...
db-backup: ## Backup database
	$(SYMFONY_CONSOLE) db-tools:backup
.PHONY: db-backup
#---------------------------------------------#
